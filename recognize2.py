...
import numpy as np
import json
import easyocr  # local AI OCR library

NUM_FRAMES = 5      # capture 5 frames per camera and pick sharpest
WAIT_TIME = 1       # seconds between frames for auto-adjust
CROP_CENTER = True  # boolean for crop center if card is centered

# use openCV for image processing

# measure sharpness (higher = sharper)
def measure_sharpness(image):
    return cv2.Laplacian(image, cv2.CV_64F).var()

# preprocess image for easier occipital character recognition or AI recognition
...
# initialize easyocr reader in English
...
for cam_num in [0, 1]: 
    ...
    try:
        ...
        if frames:
            ...
            # use easyocr to extract text from processed image
            try:
                ocr_results = reader.readtext(proc_img)
                extracted_text = "\n".join([line[1] for line in ocr_results])  # join all detected text lines
            except Exception as e:
                # fallback if easyocr fails
                extracted_text = f"<easyocr failed: {str(e)}>"

            # attempt to "parse" text into json
            text_json = {"lines": [{"text": line, "words": line.split()} for line in extracted_text.splitlines()]}
            ...
            print(f"[{timestamp}] Camera {cam_num} captured no frames!")

    finally:
        # finally ensures the camera is always stopped, even if an error occurs above
        picam2.stop()
